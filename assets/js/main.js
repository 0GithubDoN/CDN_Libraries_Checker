...
function createResultCard(url, status, details, responseTime) {
    ...
    card.innerHTML = `
        <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 break-all">${url}</h3>
    ...
    for (const url of urlsInfo.urls) {
        try {
            ...
            validationResults.push({
                ...
                responseTime: responseTimeCheck.checked ? responseTime : null,
            ...
            validationResults.push({
                url,
                status: 'error',
                details: error.message || 'Failed to access resource',
                timestamp: new Date().toISOString()
            });
        }
    }
...
window.updateUrl = (oldUrl, newUrl) => {
    ...
    // Escape special characters in the URL for use as a selector
    const escapedUrl = CSS.escape(oldUrl);
    const card = document.querySelector(`[data-url="${escapedUrl}"]`);
    
    if (!card) {
        console.error('Card element not found for URL:', oldUrl);
        showToast('Error updating URL display');
        return;
    }
    ...
    // Find and update the URL display
    const urlDisplay = card.querySelector('h3.break-all');
    if (urlDisplay) {
